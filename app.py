...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50/30 text-slate-800 font-sans selection:bg-indigo-100 selection:text-indigo-900 pb-20">
      
      {/* 1. Instruction Bar - Sleek & Dismissible */}
      {showInstructions && (
        <div className="bg-slate-900 text-slate-300 text-[11px] py-2 px-6 flex justify-between items-center shadow-sm relative overflow-hidden">
      ...
      <div className="max-w-[1600px] mx-auto p-6 md:p-10 space-y-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-3">
            <div className="bg-white rounded-3xl shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07)] border border-slate-100 h-full min-h-[240px] overflow-hidden relative group transition-all hover:shadow-lg">
          ...
          {/* Chart Card */}
          <div className="lg:col-span-6 bg-white rounded-3xl shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07)] border border-slate-100 p-6 flex flex-col relative overflow-hidden">
          ...
          {/* Stats Card */}
          <div className="lg:col-span-3 bg-white rounded-3xl shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07)] border border-slate-100 p-6 flex flex-col justify-center items-center text-center relative overflow-hidden group">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-slate-200 p-8 flex flex-col md:flex-row gap-8">
              {/* Circular Progress */}
              <div className="flex-shrink-0 flex flex-col items-center justify-center bg-slate-50 rounded-2xl p-6 border border-slate-100 w-full md:w-auto">
                 <div className="w-32 h-32 relative flex items-center justify-center">
                    <svg className="w-full h-full transform -rotate-90 drop-shadow-sm">
                      <circle cx="64" cy="64" r="56" stroke="#e2e8f0" strokeWidth="6" fill="none" className="opacity-50" />
                      <circle 
                        cx="64" cy="64" r="56" stroke="url(#gradientWeekly)" strokeWidth="6" fill="none" strokeLinecap="round"
                        strokeDasharray={351} 
                        strokeDashoffset={351 - (351 * weeklyProgress) / 100}
                        className="transition-all duration-1000 ease-out"
                      />
                      <defs>
                        <linearGradient id="gradientWeekly" x1="0%" y1="0%" x2="100%" y2="0%">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      <span className="text-3xl font-bold text-slate-700">{weeklyProgress}%</span>
                      <span className="text-[10px] uppercase font-bold text-slate-400 tracking-wider mt-1">Done</span>
                    </div>
                 </div>
              </div>
              ...
              <div className="flex-1 space-y-4 w-full">
                {[1, 2, 3, 4, 5].map((weekNum) => (
                  <div key={weekNum} className="group border border-slate-100 rounded-xl overflow-hidden transition-all hover:shadow-md hover:border-slate-200 bg-white">
                    <div className={`px-4 py-2.5 ${WEEK_COLORS[weekNum].bg} flex justify-between items-center`}>
                       ...
                       <div className="flex gap-1">
                          {weeklyHabits[weekNum].map((t, i) => (
                            <div key={i} className={`w-1.5 h-1.5 rounded-full ${t.completed ? WEEK_COLORS[weekNum].text.replace('text-', 'bg-') : 'bg-white/50'}`}></div>
                          ))}
                       </div>
                    </div>
                    <div className="p-3 space-y-2">
                       {weeklyHabits[weekNum].slice(0, 3).map((task, idx) => (
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="flex-1 flex flex-col">
              ...
              <div className="bg-white rounded-3xl shadow-sm border border-slate-200 p-1 flex-1 relative group hover:shadow-md transition-shadow">
